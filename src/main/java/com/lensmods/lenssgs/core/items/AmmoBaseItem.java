    ...
    public Component getName(ItemStack pStack) {
        ...
        if (maybe != null) {
            ...
            for(GunPartHolder part : maybe.getPartList()) {
                switch (part.getName()) {
                    case AllowedParts.ROUND -> round = part.getGunPartName().getString();
                    case AllowedParts.CASING -> casing = part.getTotalName().getString();
                    case AllowedParts.PROPELLANT -> prop = part.getGunPartName().getString();
                }
            }
            return Component.translatable(prop).copy().append(LenUtil.spaceAppend(round)).append(LenUtil.spaceAppend(casing));
        }
